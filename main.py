...
def get_root_folder(token):
    logging.info(f"Looking for root folder '{ONEDRIVE_ROOT_FOLDER}'")

    url = f"{GRAPH}/users/{MICROSOFT_DRIVE_USER}/drive/root/children"
    ...
    for item in r.json().get("value", []):
        if item["name"] == ONEDRIVE_ROOT_FOLDER:
            logging.info(f"Root folder found id={item['id']}")
            return item["id"]

    logging.info("Root folder not found, creating")

    r = requests.post(
        url,
        json={"name": ONEDRIVE_ROOT_FOLDER, "folder": {}},
...
def get_candidate_folder(token, root_id, candidate_name):
    logging.info(f"Looking for candidate folder '{candidate_name}'")

    url = f"{GRAPH}/users/{MICROSOFT_DRIVE_USER}/drive/items/{root_id}/children"
    ...
    for item in r.json().get("value", []):
        if item["name"] == candidate_name:
            logging.info(f"Candidate folder exists id={item['id']}")
            return item["id"]

    logging.info("Candidate folder not found, creating")
...
def process(payload):
    try:
        ...
        for att in valid_attachments:
            ...
            upload_url = (
                f"{GRAPH}/users/{MICROSOFT_DRIVE_USER}"
                f"/drive/items/{candidate_folder_id}:/{filename}:/content"
            )
        ...
        r = requests.post(
            f"{GRAPH}/users/{MICROSOFT_DRIVE_USER}"
            f"/drive/items/{candidate_folder_id}/createLink",
        ...
        doc_id = models.execute_kw(
            ...
            [{
                ...
                "type": "url",
                "url": web_url,
                "folder_id": recruitment_folder_id
            }]
        )
